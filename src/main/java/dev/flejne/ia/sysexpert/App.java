...
public class App implements Ihm {
	...
	private static final List<String> regles = Arrays.asList("R1:  IF (Ordre=3(Quel est l'ordre?)) THEN Triangle",
	...
	public void afficherFaits(List<Fait<?>> faits) {
		...
		afficheFaits += faits.stream().filter(f -> f.getNiveau() != 0).sorted(parNiveauDecroissant())
				.map(Fait::toString).collect(Collectors.joining("\n"));
